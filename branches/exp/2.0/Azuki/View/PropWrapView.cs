	...
	class PropWrapView : PropView
	{
		...
		public override Point GetVirPosFromIndex( IGraphics g, int lineIndex, int columnIndex )
		{
			...
			if( 0 < columnIndex )
			{
				...
				LineLogic.GetLineRange( Document.InternalBuffer, PLHI, lineIndex, true, out lineBegin, out lineEnd );
		...
		public override int GetIndexFromVirPos( IGraphics g, Point pt )
		{
			...
			if( 0 < pt.X )
			{
				...
				LineLogic.GetLineRange( Document.InternalBuffer, PLHI, lineIndex, false, out begin, out end );
